...
export const listLeadPipeline = protectedProcedure.input(listLeadsSchema).query(async ({ ctx, input }) => {
  const leads = await ctx.crmRepository.leads.list({ userId: ctx.auth.user.id, search: input.search, stage: input.stage, includeDeleted: input.includeDeleted, includeConverted: input.includeConverted });
