...
export const updateLeadStage = protectedProcedure.input(updateLeadStageSchema).mutation(async ({ ctx, input }) => {
  ...
  const lead = await ctx.crmRepository.leads.update({ userId: ctx.auth.user.id, id: input.id, fields: { stage: input.stage }, now: new Date(), expectedStage: before.stage });
  if (!lead) {
    const current = await ctx.crmRepository.leads.getById({ userId: ctx.auth.user.id, id: input.id });
    if (!current || current.deletedAt) {
      throw notFound("Lead not found.");
    }
    if (current.convertedAt && input.stage !== "won") {
      throw conflict("Converted leads must remain in the won stage.");
    }
