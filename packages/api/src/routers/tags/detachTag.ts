...
export const detachTag = protectedProcedure.input(entityTagSchema).mutation(async ({ ctx, input }) => {
  ...
  if (detached && input.entityType === "client") {
    ...
    await ctx.eventService.emitApi({ type: "project.updated", userId: ctx.auth.user.id, entity: { type: "project", id: input.entityId }, payload: { tagId: input.tagId, action: "tag.detached" } });
  }
  return { detached };
});
